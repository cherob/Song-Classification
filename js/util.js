...
module.exports = {
  ...
  calculate: function (config) {
    console.log(` ${config.categories} loaded categories`);
    console.log(`   with  ${config.files_per_class} files each,`);
    console.log(`   every file is ${config.sample_length} seconds long,`);
    console.log(`   divided into ${config.samples_per_file} samples`);
    console.log(`   with (${config.nfilt}x${config.num_ceps}) (filters, cepstal)`);
    console.log(` -> results in ${config.files_len} data records`);
    console.log(`    ID : ${config.id} MODE: ${config.modes[config.mode]}`);
  },
  shape: (A) => ([A.length || 0,
